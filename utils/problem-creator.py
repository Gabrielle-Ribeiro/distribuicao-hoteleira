...
with open('src/problem.pddl', 'w') as file:
    ...
    for i in range(total_rooms):
        file.write(f'           (is-available room{i}{rooms[i][0]})\n')
        if rooms[i][0] == 'C':
            file.write(f'           (couple-room room{i}{rooms[i][0]})\n')
        elif rooms[i][0] == 'D':
            file.write(f'           (double-room room{i}{rooms[i][0]})\n')
        elif rooms[i][0] == 'T':
            file.write(f'           (triple-room room{i}{rooms[i][0]})\n')
        elif rooms[i][0] == 'Q':
            file.write(f'           (quadruple-room room{i}{rooms[i][0]})\n')
        file.write(f'           (= (price room{i}{rooms[i][0]}) {rooms[i][1]})\n')   
